...
const PYTHON_EXT_LABEL = 'Python Script';
...
function activate(
  ...
  commands.addCommand(CommandIDs.runFile, {
      ...
      return Promise.all(
        toArray(
          map(currentWidget.selectedItems(), item => {
            ...
            if (fileNameExt[fileNameExt.length-1] === SHELL_EXT) {
            ...
            // }
            else if (fileNameExt[fileNameExt.length-1] === PYTHON_EXT) {
    ...
    // label: `Run script('.sh','.py','.sbatch') `,
    label: `Run script('.sh','.py') `,
  });
...
function prepareLauncher(launcher: any) {
  launcher.add({
    ...
    args: {
      displayIcon: true,
      iconClass: `${ICON_CLASS} ${PYTHON_ICON_CLASS}`,
      ext: PYTHON_EXT,
      extLabel: PYTHON_EXT_LABEL
    },
    rank: 2
  });
